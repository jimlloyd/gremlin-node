...
GraphWrapper.prototype.addVertex = function (properties, callback) {
  ...
  var arrayList = this.java.newArray('java.lang.String', propArray);

  var deferred = Q.defer();

  txn.addVertex(arrayList, function (err, v) {
    if (err)
...
GraphWrapper.prototype.V = function () {
  ...
  var txn = this._getTransaction();
  return this.gremlin.wrapPipeline(txn.VSync());
};
...
GraphWrapper.prototype.E = function () {
  ...
  assert(args.length === 0);
  var txn = this._getTransaction();
  return this.gremlin.wrapPipeline(txn.ESync());
};
