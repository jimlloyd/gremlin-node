...
TraversalWrapper.prototype._toGroovyFunction = function (groovy, FunctionClass) {
  var self = this;
  FunctionClass = FunctionClass || this.gremlin.GroovyPredicate;
  assert.ok(self._isClosure(groovy));
  var closure = self.gremlin.getEngine().evalSync(groovy);
  return new FunctionClass(closure);
};

TraversalWrapper.prototype._toGroovyFunctions = function (groovyStrings, FunctionClass) {
  var self = this;
  var functions = groovyStrings.map(function (groovy) {
    return self._toGroovyFunction(groovy, FunctionClass);
  });

  // Put them in a Java array of the appropriate type.
  FunctionClass = FunctionClass || this.gremlin.GroovyPredicate;
  var functionClassName = FunctionClass.class.getNameSync();
  return self.gremlin.java.newArray(functionClassName, functions);
};
...
TraversalWrapper.prototype._javify = function (arg) {
  if (arg.unwrap) {
    ...
    // Ideally we could just do something like the following. But GroovyPredicate is only appropriate in some cases.
    // We probably have to dispatch correctly for at least Function and Predicate types.
    var predicate = this._toGroovyFunction(arg);
    return predicate;
  }
  return arg;
};
