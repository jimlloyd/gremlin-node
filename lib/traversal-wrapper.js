...
TraversalWrapper.prototype._toGroovyFunctions = function (args, functionType) {
  var self = this;
  functionType = _.isString(functionType) ? functionType : 'GPredicate';
  var allowedTypes = ['GBiConsumer', 'GBiFunction', 'GBiPredicate', 'GConsumer', 'GFunction', 'GPredicate', 'GSupplier'];
  assert.ok(_.indexOf(allowedTypes, functionType) !== -1);
  var functionClassName = 'com.tinkerpop.gremlin.groovy.function.' + functionType;
  var FunctionClass = self.gremlin.java.import(functionClassName);
  args = args.map(function (f) {
    assert.ok(self._isClosure(f));
    return new FunctionClass(self.gremlin.getEngine().evalSync(f));
  });
  return self.gremlin.java.newArray(functionClassName, args);
};
...
TraversalWrapper.prototype._javify = function (arg) {
  if (arg.unwrap) {
    ...
    // Update: see _toGroovyFunctions above, and how it is used to implement select() below.
...
TraversalWrapper.prototype._jsify = function (arg) {
  ...
  if (arg.longValue) {
    ...
    return arr;
  } else if (this.gremlin.isType(arg, 'java.lang.Object')) {
    return arg;
  }
  return arg;
};
...
TraversalWrapper.prototype.addInE = function (edgeLabel, stepLabel, props) {
  this.traversal.addInESync(edgeLabel, stepLabel, this.gremlin.propertiesToVarArgs(props));
  return this;
};

TraversalWrapper.prototype.addOutE = function (edgeLabel, stepLabel, props) {
  this.traversal.addOutESync(edgeLabel, stepLabel, this.gremlin.propertiesToVarArgs(props));
  return this;
};

TraversalWrapper.prototype.addBothE = function (edgeLabel, stepLabel, props) {
  this.traversal.addBothESync(edgeLabel, stepLabel, this.gremlin.propertiesToVarArgs(props));
  return this;
};

TraversalWrapper.prototype.addE = function (direction, edgeLabel, stepLabel, props) {
  this.traversal.addESync(direction, edgeLabel, stepLabel, this.gremlin.propertiesToVarArgs(props));
  return this;
};
...
// There are four variants of select in the TinkerPop3 API:
// 1. default <E2> GraphTraversal<S,Map<String,E2>>	select(Function... stepFunctions)
// 2. default <E2> GraphTraversal<S,Map<String,E2>>	select(List<String> labels, Function... stepFunctions)
// 3. default <E2> GraphTraversal<S,E2>	select(String label)
// 4. default <E2> GraphTraversal<S,E2>	select(String label, Function stepFunction)
// The most general case is #2; the others are redundant special cases, so we implement just case #2.
// To call select with a single label, use `select(['label'], [])`, or just `select(['label'])`.
// To call select with a single function, use `select([], ['{it->...}'])`, or `select(null, ['{it->...}'])`.
TraversalWrapper.prototype.select = function () {
  var labels = arguments[0] || [];
  var functions = arguments[1] || [];

  if (arguments.length === 0) {
    assert.ok(_.isArray(labels));
    assert.ok(_.isArray(functions));
  }
  else if (arguments.length === 1) {
    if (_.isString(labels)) {
      if (this._isClosure(labels)) {
        functions = [ labels ];
        labels = [];
      }
      else {
    ...
    if (!_.isArray(labels)) labels = [ labels ];
    if (!_.isArray(functions)) functions = [ functions ];
  }
  else if (arguments.length > 2) {
    assert.ok(_.isArray(labels));
    functions = Array.prototype.slice.call(arguments).slice(1);
  }

  assert.ok(_.isArray(labels));
  assert.ok(_.isArray(functions));

  var labelArgs = this.gremlin.toListSync(labels);
  var funcArgs = this._toGroovyFunctions(functions, 'GFunction');

  this.traversal.selectSync(labelArgs, funcArgs);
  return this;
};
...
TraversalWrapper.prototype._asJSON = function (elem) {
  var self = this;
  var java = this.gremlin.java;
  if (!_.isObject(elem)) {
...
// `includeEdgeClosure` must be a string expressing a Groovy closure of an edge predicate.
...
TraversalWrapper.prototype.subgraph = function (includeEdgeClosure, callback) {
  var self = this;
  assert.ok(self._isClosure(includeEdgeClosure));
  var edgePredicate = self._javify(includeEdgeClosure);
